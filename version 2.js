...
function runMarmoUI()
{
	...
	var reload_time = 0.1; // Time to wait until reload, in seconds
	var update_location = "https://raw.github.com/lishd/MarmoUI/master/updater.css"; //latest version inside a CSS
	var update_download = "https://userscripts-mirror.org/scripts/source/157749.user.js"; //Download page
  ...
  var yellow_tint = [255, 255, 30];
	...
	function queueAsyncReload(element, requestURL, callback, countdown)
	{
		...
		else
		{
			window.setTimeout(function() 
			{
				queueAsyncReload(element, requestURL, callback, countdown - 0.05);
			}, 50);
		}
	}
...
//           console.log();
	...
	function applyChangesSubmissionPage()
	{
		...
		$("p:contains('points for')").each(function(index, value){var inner = $(value).html(); $(value).replaceWith("<h3 style='color:#fc3;font-weight:bold;background:rgba(50,50,50,0.5);'>" + inner + "</h3>");});
		//Add sorting to table
		addTableSorting();
